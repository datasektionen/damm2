...
export const BoxDataGrid = ({ boxes, setBoxes }: Props) => {
    ...
    const deleteBoxes = useCallback(() => {
        ...
        axios.post("/api/storage/boxes/delete", {
    ...
    const boxColumns: Column<BoxRow>[] = useMemo(() => {
        return [
            ...
            { key: "id", name: "ID", width: 30 },
            {
                key: "name",
                name: "Namn",
                editor: ({ row }) => <CellStringInput initial={row.name} row={row} onSave={updateBoxName} />,
                formatter: ({ row }) => <div title="Dubbelklicka för att redigera" style={{ cursor: "pointer" }}>{row.name}</div>
            },
    ...
    const updateBoxName = async (rowId: number, value: string) => {
        return axios.put("/api/storage/box/name", {
            boxId: rowId,
            name: value,
        })
            .then(() => {
