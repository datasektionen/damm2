...
import { useShouldHidePatch } from '../../../../components/Patch/Patch';
...
export const DetailsView: React.FC<Props> = ({
  patch,
  onEditClick,
  onClose,
  fetchPatches,
  ...
  const { isAffectedByDarkmode, tagsThatApplyDarkMode } = useShouldHidePatch();
  ...
  const deleteFile = (name: string) => {
    ...
    axios
      .delete(
        url(
          `/api/files/file/?name=${encodeURIComponent(name)}&id=${
            patch.id
          }&type=${type}`
        ),
        {
  ...
  return (
    <>
      <Left>
        ...
        {isAdmin && (
          <Button label="Redigera" onClick={onEditClick} disabled={loading} />
        )}
        {isAdmin &&
          type === 'patch' &&
          patch.files &&
            ...
            <Files>
              ...
              {patch.files.map((f: string) => {
                const completeUrl = url(
                  `/api/files/get/${encodeURIComponent(
                    f
                  ...
                  <div key={'patch-file-' + f}>
                    ...
                    <Thrash
                      onClick={() => {
                        ...
                        if (
                          window.confirm(
                            'Är du säker? Filen tas bort permanent från AWS S3.'
                          )
                        )
                          deleteFile(f);
                      }}
        ...
        {type === 'artefact' && patch.files.length !== 0 && (
          <Files>
            ...
            {patch.files.map((f: string) => {
              const completeUrl = url(
                `/api/files/get/unprotected/${encodeURIComponent(f)}`
              );
              return (
                <div key={'patch-file-' + f}>
                  <a
      ...
      <Right>
        ...
        <Meta>
          ...
          {patch.createdBy.length !== 0 && (
            <div>
              Skapad av{' '}
              <Creators>
                {patch.createdBy.map((c) => c.name).join(', ')}
              </Creators>
            </div>
          )}
        </Meta>
        <Content>
          ...
          {isAdmin && isAffectedByDarkmode(patch) && (
            <p style={{ fontSize: 12 }}>
              Märket kommer gömmas vid mörkläggning då den har taggar som
              tillhör mottagningskategorin:{' '}
              {tagsThatApplyDarkMode(patch)
                .map((x) => `"${x.name}"`)
                .join(', ')}
            </p>
          )}
          ...
          {isAdmin && (
            <StorageItems>
              <StorageItem title="Låda">
                ...
                <span>{patch.bag?.box.name ?? '?'}</span>
              </StorageItem>
              <StorageItem title="Påse">
                <i className="fa-solid fa-bag-shopping"></i>
                <span>{patch.bag?.name ?? '?'}</span>
              </StorageItem>
            </StorageItems>
          )}
          <StorageItems>
