...
export const EditDetails: React.FC<Props> = ({
      ...
      .then(async (res) => {
        if (files.length !== 0) {
          await uploadFiles(
            editState.id,
            files[0],
            type === 'patch' ? 'patch-files' : 'artefact-files',
            type
          );
        }
        await fetchPatches();
  ...
  const onCreatePerson = useCallback(async (query) => {
  ...
  return (
    <StyledEditDetails ref={ref}>
      ...
      <TagSelector
        ...
        setSelectedTags={(next: ITag[]) =>
          setEditState({ ...editState, tags: next })
        }
        disabled={loading}
        query=""
      />
          ...
          disabled={
            ...
            (patch === editState &&
              ...
              JSON.stringify(creators) ==
                JSON.stringify(editState.createdBy.map((c) => `${c.id}`)))
          }
          isLoading={loading}
        />
      </BRow>
      ...
      <DeleteBox>
        <h4 style={{ color: Theme.palette.red }}>
          <b>Radera märket</b>
        </h4>
        <p>
          Detta kan inte ångras. Skriv "<b>Radera permanent</b>" nedan för att
