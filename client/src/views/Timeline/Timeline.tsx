...
import { StyledTimeline, TimelineBody, Cards, Year, Line, CardWrapper, LineEnd, LineStart } from './style';
...
const templates = {
    SM_DM: {
        ...
        template: (data: IEvent, i: number, onEditClick: any) => 
            <CardWrapper
                index={i}
                key={"card-" + moment(data.date).year() + "-" + i}
            >
                <SM {...data} protocol={data.protocol ?? ""} index={i} onEditClick={onEditClick} />
            </CardWrapper>
    },
    ANNIVERSARY: {
        title: "Årsdagar",
...
export const Timeline: React.FC = props => {
    ...
    useEffect(() => {
        (async () => {
            ...
            setEvents(events.data.body)
            setYears(eventsPerYear(events.data.body))
        })()
    }, [])
