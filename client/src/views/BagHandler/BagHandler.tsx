...
export const BagHandler = () => {
  ...
  useEffect(() => {
    // boxes is empty in first render, set selectedBox once when boxes is filled with data
  ...
  const getBoxes = useCallback(() => {
    axios.get('/api/storage/box/all').then((res) => {
      setBoxes(res.data.body);
    });
  }, []);

  const getBags = useCallback(() => {
    axios.get('/api/storage/bag/all').then((res) => {
  ...
  return (
    <>
      ...
      >
        {boxes.map((b) => (
          <option key={'box-' + b.id} value={b.id}>
            {b.name}
          </option>
        ))}
      </select>
      ...
      <BagDataGrid bags={bags} boxes={boxes} setBags={setBags} />
    </>
  );
};
